...
/**
 * @brief Setup ETH context for the given eth_context.
 *
 * This function initializes and configures a net context for ETH for the specified
 * eth_context. It performs the following steps:
 * 1. Configures the server address.
 * 2. Sets the ETH receive callback function.
 *
 * @param ctx Pointer to a eth_context structure.
 * @return CSP_ERR_NONE on success, CSP_ERR_DRIVER on failure.
 */
int csp_eth_setup_context(eth_context_t * ctx) {
	int ret;
	...
	if (ret < 0) {
		LOG_ERR("Setting ETH rx callback failed (%d)\n", ret);
...
int csp_eth_init(const char * device, const char * ifname, int mtu, unsigned int node_id, bool promisc, csp_iface_t ** return_iface) {
	...
	if (csp_eth_setup_context(ctx) != CSP_ERR_NONE) {
...
int csp_eth_stop_rx(csp_iface_t * iface) {
	...
	if (ctx->eth_ctx) {
		/* Stop the rx callback, no more eth packet are received */
		net_context_recv(ctx->eth_ctx, NULL, K_NO_WAIT, NULL);
		LOG_INF("ETH reception stopped");

		/* Free the net context, eth stopped */
